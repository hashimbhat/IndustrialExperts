...
{
    public partial class ConAppointmentReq : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    string cont = Session["Email"].ToString(); // Get the value of session variable as a string
                    string sta = "Requested";
                    using (SqlConnection con = new SqlConnection(str))
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT ConId,Fname,Email,PhoneNo,WorkDesc,StartDate,AddressSite,Status FROM AppointmentsCon WHERE EmpEmail=@conmail AND Status=@sta", con))
                        {
                            cmd.Parameters.AddWithValue("@sta", sta);
                            cmd.Parameters.AddWithValue("@conmail", cont);
                            con.Open();
                            ...
                            AppointmentReqConGrid.DataSource = dataTable;
                            AppointmentReqConGrid.DataBind();
                            con.Close();
        ...
        protected void AppointmentReqConGrid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Approve")
            {
                ...
                using (SqlConnection con = new SqlConnection(str))
                {
                    using (SqlCommand cmd = new SqlCommand(" UPDATE AppointmentsCon SET Status=@stat,Amount=@amou WHERE ConId=@email", con))
                    {
                        cmd.Parameters.AddWithValue("@email", cus);
