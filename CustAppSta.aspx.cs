...
{
    public partial class CustAppSta : System.Web.UI.Page
    {
                ...
                if (Session["Email"] != null)
                {
                    ...
                    using (SqlConnection con = new SqlConnection(str))
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT EmpId,EmpEmail,WorkDesc,StartDate,AddressSite,Status,Amount FROM AppointmentsEmp WHERE Email=@empmail AND Status=@sta ", con))
                        {
                            ...
                            CustAppStaGrid.DataSource = dataTable;         
                            CustAppStaGrid.DataBind();
                            con.Close();
        ...
        protected void CustAppStaGrid_RowCommand(object sender, GridViewCommandEventArgs e)
        {
